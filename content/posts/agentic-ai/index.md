...
{{< callout type="note" >}}
**A later correction, six months on.** The 120-point margin above is not a fact about the two architectures. The benchmark handed the Bayesian agent each question's category as a perfect oracle, straight into its per-category reliability table; the LangChain agent got the question text and had to work out for itself what kind of question it was looking at. That is not a hard comparison to win. Take the oracle away and a frontier model wins outright on score — and a one-line greedy ablation, query the single most reliable tool and submit and stop with no value-of-information calculation anywhere, beats the full Bayesian agent too.

The argument is in better shape than its headline number, which is fortunate, because I would still make it. It never rested on the size of the margin. An agent that queries 3.22 tools per question without regard to whether the query is worth its cost is not weighing anything, and prompting it to be "careful" made it query more rather than less. Those are measurements *within* the LangChain family, under identical conditions, and the oracle does not touch them. The Bayesian substrate also kept the cost-efficient end of the frontier in the fair re-run. What fell is the sentence I most enjoyed writing, and it fell because the benchmark was designed by the same person who was hoping for that result. The full account is in [The Ablation That Beat the Agent](/posts/credence-paper/).
{{< /callout >}}
