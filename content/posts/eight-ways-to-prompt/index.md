...
{{< callout type="note" >}}
**A later correction, replacing a hedge that was too gentle with itself.** This benchmark used text-based LLM parsing and prompted variants of a single model. Both are worth knowing; neither is what broke. What broke is the headline this post inherits. The Bayesian agent was handed each question's category as a perfect oracle, straight into its per-category reliability table; the LLM variants read the question and had to infer it. Remove that asymmetry and a frontier model wins on score, and a one-line greedy ablation --- query the most reliable tool, submit, stop, no value-of-information calculation anywhere --- beats the full Bayesian agent. "Twelve times more effective" is a number from a comparison I built and then won. It should not be read as a fact about the architectures.

What this post actually measured is in better shape, because the gradient is a comparison *among* the LLM variants, and they all faced identical conditions. Each technique helps. The more thorough the prompting, the more queries it buys. And the cost-efficiency ordering did survive the fair re-run, even though the specific figures here came from the rigged configuration. The category error is untouched and is the part worth keeping: a model can write the sentence describing a cost-benefit calculation without ever performing one. The full account is in [The Ablation That Beat the Agent](/posts/credence-paper/).
{{< /callout >}}
