...
This benchmark was intended as the headline experiment of a paper. The same decision-theoretic core that selects tools for simulated QA also [routes real LLM traffic in production](/posts/ninety-six-percent-cheaper/), with comparable results at a ninety-six percent lower cost. The mechanism travels. The arithmetic is the same arithmetic.

{{< callout type="note" >}}
**A later correction, in the interest of not letting the good number stand unchallenged.** This benchmark used text-based LLM parsing rather than native tool-calling, and the LLM agents were prompted variants of a single model. Both are worth knowing; neither is what broke.

The comparison above is unfair in a specific and since-measured way. The Bayesian agent received each question's category as a perfect oracle, handed straight to its per-category reliability table, while the LLM agents had to infer the category from the question text. And roughly thirty of the fifty questions are answerable cold by a frontier model with no tool call at all, so the benchmark could not separate the value of *tool selection* from the value of parametric recall. Remove the worst of that asymmetry and the ordering reverses: Haiku 4.5 scores +445.5 against the Bayesian agent's +163.7.

Worse for the argument as I made it, a one-line greedy ablation --- query the single most reliable tool, submit, stop, no value-of-information calculation anywhere --- beats the full Bayesian agent by 25.75 points under fair conditions, and keeps beating it under every credit-assignment rule tested. The ablation table I published elsewhere, the one that said removing VOI costs 78.1 points, was computed under oracle categories and flips sign without them.

What survives is narrower and conditional. The Bayesian substrate --- reliability learning plus category inference --- still owns the cost-efficient frontier in both conditions, and VOI still dominates a free local model on queries and score together. Optimism-greedy is itself a Bayesian policy, so what fell is not decision theory against prompting; it is the narrower claim that the VOI *action layer* specifically earns its cost once categories have to be inferred. And the arithmetic this post is actually about --- that a scoring function with costs and penalties makes accuracy the wrong objective --- never depended on which agent won. The full account, including why the paper never went to arXiv, is in [The Ablation That Beat the Agent](/posts/credence-paper/).
{{< /callout >}}
