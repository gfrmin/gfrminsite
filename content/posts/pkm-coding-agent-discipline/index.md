...
The companion post on what the foundation is *for* — the PKM project, the failing-tools critique, the Karpathy comparison, the four canonical questions that started this whole thing — is [here](/posts/pkm-phase-1/). The code now lives in the [life-agent monorepo](https://github.com/gfrmin/life-agent) as `src/pkm`. The two artefacts worth reading, for anyone planning a similar exercise, are `docs/pkm/SPEC.md` (the contract) and `src/pkm/CLAUDE.md` (the leash).
